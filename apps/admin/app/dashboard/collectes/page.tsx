...
export default function CollectesPage() {
  ...
  const { canAccessDriverSchedule, loading: roleLoading, profile } = useUserRole();

  // Redirect if no access
  useEffect(() => {
    if (!roleLoading && !canAccessDriverSchedule) {
      router.push('/dashboard');
    }
  }, [canAccessDriverSchedule, roleLoading, router]);
  ...
  return (
    <>
      <Topbar
        title="Collectes & Livraisons"
        subtitle={`Bonjour ${profile?.full_name || 'Chauffeur'} - Vos missions du jour`}
      />
      <PageContainer>
        <div className="space-y-6">
          ...
          <section>
            <div className="flex items-center gap-2 mb-4">
              <h2 className="text-xl font-semibold text-vdDark">Aujourd'hui</h2>
            ...
            {!loading && !error && todayAppointments.length === 0 && (
              <Card className="p-6 text-center">
                ...
                <p className="text-vdMuted">Aucune mission prévue pour aujourd'hui.</p>
              </Card>
            )}
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="ml-auto text-sm font-semibold text-vdPrimary">{appt.slot || '—'}</span>
                      </div>
                      <p className="text-xl font-bold text-vdDark mb-1">{appt.customer_name || 'Client'}</p>
                      {appt.address && (
                        <p className="text-sm text-vdMuted line-clamp-2">{appt.address}</p>
                      )}
                    </div>
                    ...
                    <div className="flex border-t border-vdBorder">
                      {appt.customer_phone && (
                        <a
                          ...
                          className="flex-1 flex items-center justify-center gap-2 py-4 text-vdPrimary font-semibold active:bg-vdSurface transition border-r border-vdBorder"
          ...
          {upcomingAppointments.length > 0 && (
            <section>
              <div className="flex items-center gap-2 mb-4">
                <h2 className="text-xl font-semibold text-vdDark">À venir</h2>
              ...
              <div className="space-y-3">
                {upcomingAppointments.map((appt) => (
                  <Card
                    key={appt.id}
                    onClick={() => openModal(appt)}
                    className="p-4 cursor-pointer active:bg-vdSurface transition opacity-75"
                  >
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className="ml-auto text-sm text-vdMuted">{appt.slot || '—'}</span>
                    </div>
                    <p className="text-lg font-semibold text-vdDark">{appt.customer_name || 'Client'}</p>
                    {appt.address && (
                      <p className="text-sm text-vdMuted line-clamp-1 mt-1">{appt.address}</p>
                    )}
                  </Card>
                ))}
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-vdDark">{selectedAppointment.customer_name || 'Client'}</p>
                </div>
              </div>
              ...
              {selectedAppointment.customer_phone && (
                <div className="flex items-center gap-3">
                  ...
                  <a
                    ...
                    className="text-vdPrimary font-semibold"
                  >
                    {selectedAppointment.customer_phone}
                  </a>
                </div>
              ...
              <div className="flex items-center gap-3">
                <Clock className="w-5 h-5 text-vdMuted" />
              ...
              {selectedAppointment.address && (
                <div className="space-y-2">
                  <div className="flex items-start gap-3">
                    ...
                    <span className="text-vdDark">{selectedAppointment.address}</span>
                  </div>
            ...
            <div className="border-t border-vdBorder pt-4">
              <div className="flex items-center gap-3 mb-2">
                <Truck className="w-5 h-5 text-vdMuted" />
                <p className="text-xs text-vdMuted">Chauffeur assigné:</p>
              </div>
              <select
                value={selectedDriverId}
                onChange={(e) => handleDriverChange(e.target.value)}
                disabled={updating}
                className="w-full px-3 py-2 rounded-xl border border-vdBorder bg-vdSurface text-vdDark focus:outline-none focus:ring-2 focus:ring-vdPrimary/50"
              >
            ...
            <div className="border-t border-vdBorder pt-4">
              <p className="text-xs text-vdMuted mb-2">Mettre à jour le statut:</p>
      ...
      >
        <div className="space-y-3">
          <p className="text-sm text-vdMuted mb-4">{gpsAddress}</p>
          <Button
            variant="primary"
            className="w-full"
