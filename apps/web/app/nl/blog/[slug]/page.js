...
export default async function BlogPostNLPage({ params }) {
  const { slug } = await params;
  const post = getBlogPost(slug);
  if (!post) notFound();
    ...
    <main>
      <section className="bg-[#F7FAFB] py-16 md:py-24">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <Link href="/nl/blog" className="inline-flex items-center gap-2 text-[#1B3B8A] font-medium mb-8 hover:gap-3 transition-all">
      ...
      <section className="py-16 md:py-20">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-16 pt-8 border-t border-gray-200">
            <p className="text-gray-500 mb-4">Een professional nodig?</p>
            <Link href="/nl/offerte" className="inline-flex items-center gap-2 bg-airPrimary text-white font-semibold px-6 py-3 rounded-xl hover:-translate-y-0.5 transition-all">
